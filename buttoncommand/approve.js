...
module.exports = {
    ...
    async execute(interaction,client){
        if(interaction.channelId === config.adminYTChannel){
            const embed2 = new EmbedBuilder().setColor('Blue').setDescription(`你在 ${config.servername} 的 YouTube會員 審核已經通過，現在可以觀看會限頻道了`)
            ...
            const finish = new ButtonBuilder().setCustomId('finish').setLabel(`審核通過 by ${interaction.user.tag}`).setStyle(ButtonStyle.Secondary).setDisabled(true)
            ...
            const embed2 = new EmbedBuilder().setColor('Blue').setDescription(`你在 ${config.servername} 的 TwitCasting會員 審核已經通過，現在可以觀看會限頻道了`)
            const oldEmbedFooter = interaction.message.embeds[0].footer.text
            // console.log(oldEmbedFooter);
